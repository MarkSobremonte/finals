...
const LoginPage = () => {
    ...
    const loginMutation = useMutation({
        ...
        onSuccess: (data) => {
            console.log('Sumakses ', data);
        },
        onError: (error: any) => {
            console.error('Login Failed:', error);
            const errorMessage = error.response?.data?.message || 'Failed ulitin mo nalang';
            alert(errorMessage);
        }
    })
    ...
    const registerMutation = useMutation({
        ...
        onSuccess: (data) => {
            console.log('Shessshh suksespuli regiter:', data);
            setIsLogin(true);
            setRegisterForm({
                name: "",
                email: "",
        ...
        onError: (error: any) => {
            ...
            const errorMessage = error.response?.data?.message || 'aww kawawa failed';
            alert(errorMessage);
        }
    })
    ...
    const handleRegisterSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (registerForm.password !== registerForm.confirmPassword) {
            alert("huyy di nag mamatch yung password");
            return;
        }
            ...
            <div className='bg-white rounded-2xl shadow-2xl w-full max-w-md p-8'>
                <div className='text-center mb-8'>
                    <h1 className='text-3xl font-bold text-gray-800 mb-2'>
                        {isLogin ? 'Walang welcomeback' : 'Create kana'}
                    </h1>
                ...
                <div className='flex mb-6 bg-gray-100 rounded-lg p-1'>
                    <button onClick={() => setIsLogin(true)} className={`flex-1 py-2 px-4 rounded-md text-sm font-medium transition-all ${isLogin ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-800' }`}>
                        Sign In
                    </button>
                    <button onClick={() => setIsLogin(false)} className={`flex-1 py-2 px-4 rounded-md text-sm font-medium transition-all ${ !isLogin  ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-800'}`} >
                        Register
                    </button>
                </div>
                    ...
                    <form onSubmit={handleLoginSubmit} className='space-y-6'>
                        <div>
                            ...
                            <input 
                                ...
                                required
                                className='w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors' placeholder="Enter your email"/>
                        </div>
                        <div>
                        ...
                        {loginMutation.isError && (
                            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                                <p className="text-red-800 text-sm">Login failed. Please check your credentials.</p>
                            </div>
                        )}
